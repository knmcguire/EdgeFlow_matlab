...
vals = find(~isnan(velocity_tot_forward_plot));
vals = vals(3:end);
    
velocity_error_forward = abs(velocity_tot_forward_plot(vals)' - cam_Vz_frame(vals));
velocity_error_sideways = abs(velocity_tot_sideways_plot(vals)' - cam_Vx_frame(vals));
velocity_error_forward_glob = abs(velocity_tot_forward_global_plot(vals)' - cam_Vz_frame(vals));
velocity_error_sideways_glob = abs(velocity_tot_sideways_global_plot(vals)' - cam_Vx_frame(vals));
peaks_hist = peaks_hist(vals)';
matching_error_flow_t = matching_error_flow_t(vals)';
mean_distance = mean_distance(vals)';
