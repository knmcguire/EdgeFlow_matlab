...
stereoboard = 2; % select which stereoboard to choose from (1 and/or 2, see Edgeflow_prepare_data)
...
for stereoboard_type = stereoboard
    for track = chosen_tracks(stereoboard_type).track
        ...
        matching_error_flow_t(matching_error_flow_t==inf&isnan(matching_error_flow_t)) = 0;
        peaks_hist(peaks_hist==inf & isnan(peaks_hist)) = 0;
        velocity_error_forward(velocity_error_forward==inf & isnan(velocity_error_forward)) = 0;
        velocity_error_sideways(velocity_error_sideways==inf & isnan(velocity_error_sideways)) = 0;
...
figure
subplot(1,2,1),
boxplot(velocity_error_forward_glob_tot,round(mean_distance_tot*2)/2)
ylim([0 0.3])
xlim([1.5 6.5])
ylabel('Velocity Error [m/s]')
title('x')
...
subplot(1,2,2),
boxplot(velocity_error_sideways_glob_tot,round(mean_distance_tot*2)/2)
ylim([0 0.3])
xlim([1.5 6.5])
